...
async def connect(token):
    ...
    while True:
        try:
            ...
            if msg:
                main_func(MessageClass(json.loads(msg.data)['d']))
...
def buy(coin: str, investment: int) -> str:
    ...
    response = requests.post(
        url=f"{bot.base}{endpoint}", headers=headers, json=payload)
...
def main_func(message: MessageClass):
    import cProfile
    import pstats
    ...
    with cProfile.Profile() as pr:
        ...
        before = time.time()
        # test, real
        if message.channel_id not in pump_signals_channels:
            return
        print('h1')
        ...
        orderId = buy(coin, bot.investment)

        buy_time = time.time()
        print('h3')

        while True:
            coin_bought_amount = get_order(orderId)
            if coin_bought_amount != "0":
                coin_bought_amount = coin_bought_amount
                break
        print('h4')

        int1, int2 = message.content.split(
            "Gain: ")[-1].split("%")[0].split("-")
    ...
    stats.dump_stats(filename="h.prof")
