...
class Bot:
    if not os.path.exists("investment.txt"):
        open("investment.txt", "w").write("1")
    investment = int(open("investment.txt", "r").read())
...
async def connect(token):
    ...
    while True:
        try:
            ...
            if msg:
                await main_func(MessageClass(json.loads(msg.data)['d']))
...
async def buy(coin: str, investment: int) -> str:
    ...
    response = await aiohttp.ClientSession().post(url=f"{bot.base}{endpoint}", headers=headers, json=payload)
    return ((await response.json())["data"]["orderId"])


async def sell(coin: str, coin_bought: str, resale_price: str):
    endpoint = "/api/v1/orders"
    payload = {
        "clientOid": str(random.randint(1, 100000)),
        "side": "sell",
...
async def get_order(orderId, full=False):
    ...
    response = await aiohttp.ClientSession().get(url=f"{bot.base}{endpoint}", headers=headers, json=payload)
    if full:
...
async def send_message(content):
    h = await aiohttp.ClientSession().post(
        url=f"https://discord.com/api/v9/channels/{master_channel}/messages",
        headers={
...
async def main_func(message: MessageClass):
    print("received")
    print(message.content)
    if message.channel_id not in pump_signals_channels:  # test, real
        return
    print('h1')
    ...
    orderId, pr = await buy(coin, bot.investment)
    buy_time = time.perf_counter()

    print('h3')

    while True:
        coin_bought_amount = await get_order(orderId)
